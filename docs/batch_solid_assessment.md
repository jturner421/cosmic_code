...
- `allocate_batch(line)`: guards with `can_allocate` before adding the line to `_allocations`, making the method idempotent thanks to set semantics.
...
- **Dependency Inversion Principle**: Satisfied in context. `Batch` relies only on the value object `OrderLine` and standard library types, keeping high-level policy (`allocate_batch`) independent of infrastructure details. Further inversion could be pursued if external services enter the picture.
